...
import com.prography.sw.aloocustomer.databinding.ActivitySupermarketPageBinding;
...
public class SupermarketPageActivity extends AppCompatActivity implements CustomListener, View.OnClickListener {

    private ActivitySupermarketPageBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivitySupermarketPageBinding.inflate(getLayoutInflater());
    ...
    private void initViews() {
        ...
        binding.rvSupermarketPage.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                if (!binding.rvSupermarketPage.canScrollVertically(1) && binding.rvSupermarketPage.getVisibility() == View.VISIBLE
                        && !mainViewModel.isFetchingExhausted() && !mainViewModel.isFetchingNextPage()) {
                    // search the next page
                    loaded = false;
                    mainViewModel.getNextPageVendor(id, Double.valueOf(SharedPreferencesHelper.getLon(SupermarketPageActivity.this)),
                            Double.valueOf(SharedPreferencesHelper.getLat(SupermarketPageActivity.this)));
                }
            }
        });
    ...
    private void initRestaurantPageAdapter() {
        ...
        binding.rvSupermarketPage.setAdapter(adapter);
    ...
    private void showParent(int visibility) {
        if (visibility == View.VISIBLE) {
            ...
            binding.rvSupermarketPage.setVisibility(View.VISIBLE);

        } else {
            binding.loading.setVisibility(View.VISIBLE);
            binding.rvSupermarketPage.setVisibility(View.GONE);
    ...
    private void updateUi(GeneralResponse response) {
        ...
        if (response.getVendorSuperPharmsData().getItems() != null && response.getVendorSuperPharmsData().getItems().size() > 0) {
            ...
            if (oldPosition > 0) {
                binding.rvSupermarketPage.smoothScrollToPosition(oldPosition);
            }
    ...
    public void onItemClick(int position, String action) {
        switch (action) {
            case "ITEM_SUPERMARKET_PAGE":
                Intent intent = new Intent(SupermarketPageActivity.this, ProductDetailsActivity.class);
            ...
            case "highToLow":
                ...
                loaded = false;
                startVendorProcess();
                bottomSheetCardList.dismissDialog();
                break;
            case "lowToHigh":
