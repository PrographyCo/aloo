...
import com.prography.sw.aloocustomer.databinding.ActivityPharmacyBinding;
...
public class PharmacyActivity extends AppCompatActivity implements CustomListener, View.OnClickListener {
    ...
    private void initView() {
        ...
        binding.rvPharmacy.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                if (!binding.rvPharmacy.canScrollVertically(1) && binding.rvPharmacy.getVisibility() == View.VISIBLE
    ...
    private void initPharmacyAdapter() {
        ...
        binding.rvPharmacy.setAdapter(adapter);
    }
    ...
    private void showParent(int visibility) {
        if (visibility == View.VISIBLE) {
            ...
            binding.rvPharmacy.setVisibility(View.VISIBLE);

        } else {
            binding.loading.setVisibility(View.VISIBLE);
            binding.rvPharmacy.setVisibility(View.GONE);
    ...
    private void updateUi(GeneralResponse response) {
        ...
        if (response.getMainData().getItems() != null && response.getMainData().getItems().size() > 0) {
            ...
            if (oldPosition > 0) {
                binding.rvPharmacy.smoothScrollToPosition(oldPosition);
            }
    ...
    public void onItemClick(int position, String action) {
        if (action.equals("MAP_SHEET")) {
            ...
            Intent intent = new Intent(PharmacyActivity.this, PharmacyPageActivity.class);
