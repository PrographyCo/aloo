        ...
        <div class="pagetitle">
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.settings.kitchen.index') }}">kitchen types</a></li>
                    <li class="breadcrumb-item active">kitchen types list</li>
                </ol>
            </nav>
        </div>
        ...
        <section class="section dashboard">
            <div class="card p-4">
                <div class="card-header" id="create_city_div">
                    <form id="create_kitchen_form" action="{{ route('admin.settings.kitchen.store') }}" method="POST">
                        @csrf
                        <input type="hidden" value="" name="name_en" id="name_en">
                        <input type="hidden" value="" name="name_ar" id="name_ar">
                    </form>
                    <button class="rounded btn btn-primary" onclick="createKitchen()" id="create_city">add new kitchen</button>
                </div>
                <div class="card-body">
                    {{ $dataTable->table() }}
...
@push('scripts')
    <script>
        ...
        async function editKitchen($type) {
            let form = $('#edit_kitchen_form'), url = "{{ url('/admin/settings/kitchen/::type::') }}";
