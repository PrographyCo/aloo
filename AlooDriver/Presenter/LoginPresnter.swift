...
class LoginPresnter:NSObject{
    ...
    private func sendFCMToken(){
        GeneralActions.shard.monitorNetwork {
            DispatchQueue.main.async {
                if UserDefaults.standard.bool(forKey: "setToken"){
                    ProfileApiControllers.shard.firebaseToken { status, message in
                        if status{
                            UserDefaults.standard.setValue(false, forKey: "setToken")
                        }
                    }
                }
            }
